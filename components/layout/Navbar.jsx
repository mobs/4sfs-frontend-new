        ...
        <div className="flex justify-between items-center h-24">
            ...
            {NAV_ITEMS.map((item) => (
              ...
              >
                {item.children ? (
                  ...
                  <Link
                    href={item.href}
                    className="group relative block px-5 py-3 text-sm font-semibold tracking-wide text-darkgray hover:text-primary transition-colors duration-300 rounded-xl overflow-hidden"
                  >
                    {/* Elegant hover background */}
          ...
          <div className="hidden lg:flex items-center space-x-4">
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button variant="primary" className="rounded-full px-8 py-3 text-sm font-semibold shadow-lg hover:shadow-xl">
                Get Started
              </Button>
            </motion.div>
          </div>
              ...
              {NAV_ITEMS.map((item) => (
                <div key={item.label} className="border-b border-gray-100 last:border-0">
                  {item.children ? (
                    ...
                    <Link
                      href={item.href}
                      className="flex items-center space-x-2 py-3 text-darkgray font-medium hover:text-primary transition-colors"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      {item.icon && <item.icon className="text-primary" />}
                      <span>{item.label}</span>
